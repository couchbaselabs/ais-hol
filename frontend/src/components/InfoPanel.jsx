...
async def voice_chat(audio: UploadFile = File(...)):
  ...
  'agent-memory-overview': {
    title: 'Agent Memory SDK — Overview',
    subtitle: 'Persistent, searchable memory for AI agents backed by Couchbase',
    color: CB_ACCENT,
    icon: '🧠',
    what: 'The Couchbase Agent Memory SDK provides a persistent memory layer for AI agents. Agents store conversation turns and extracted facts as MemoryBlocks, then retrieve relevant context via semantic search before generating responses. Memory is scoped to a user → session hierarchy and stored as vectors in Couchbase.',
    how: [
      'Create a user and session: client.create_user(id, name) → user.create_session(session_id)',
      'Store turns: session.add_memory(messages=[ChatMessage(user_content=…, assistant_content=…)])',
      'Store facts: session.add_memory(facts=["User prefers dark roast coffee"])',
      'Retrieve context: session.search_memory(query=question, filters={"session_ids": "all"})',
      'Inject ranked MemoryBlocks into the LLM system prompt before generating a response',
    ],
    limitations: [
      'Requires a running Agent Memory server (agentmem) — the SDK is a thin HTTP client',
      'async_processing=True queues blocks for background embedding — not immediately searchable',
      'Server requires Couchbase >= 8.0.0 for vector index support',
    ],
    stack: ['Couchbase Agent Memory SDK', 'agentmem server', 'Couchbase vector search', 'FastAPI'],
    questions: [
      { label: 'What is the difference between a fact and a message memory block?', text: null },
      { label: 'How does the memory server embed and index blocks?', text: null },
      { label: 'Can I use the async client in a FastAPI app?', text: null },
    ],
  },
  'agent-memory-sessions': {
    title: 'Sessions & Memory Blocks',
    subtitle: 'User → Session → MemoryBlock resource hierarchy',
    color: CB_ACCENT,
    icon: '💾',
    what: 'Memory is organised in a three-level hierarchy: Users own Sessions, Sessions own MemoryBlocks. Each level returns a resource object you can chain further operations on. MemoryBlocks hold either a ChatMessage (user + assistant turn) or a Fact (extracted string).',
    how: [
      'create_user(user_id, name) — idempotent; returns UserResource for chaining',
      'user.create_session(session_id) — scopes all memory to this session; set blocks_ttl for auto-expiry',
      'session.add_memory(messages=[…]) — stores raw conversation turns',
      'session.add_memory(facts=[…]) — stores compact extracted facts; more token-efficient for long-term memory',
      'session.list_memories(limit, order_by) — paginated listing; session.update_memory() and delete_memory() for CRUD',
    ],
    limitations: [
      'async_processing=False blocks until embedding is complete — use True for high-throughput ingestion',
      'blocks_ttl is set at session creation and cannot be changed per-block',
      'Facts are extracted by the server LLM — quality depends on the configured LLM model',
    ],
    stack: ['AgentMemoryClient', 'UserResource', 'SessionResource', 'MemoryBlock'],
    questions: [
      { label: 'How do I set a TTL so memory expires after a session ends?', text: null },
      { label: 'What is the difference between async_processing=True and False?', text: null },
      { label: 'How do I update or correct an existing memory block?', text: null },
    ],
  },
  'agent-memory-search': {
    title: 'Semantic Memory Search',
    subtitle: 'Vector search over memory blocks with session and annotation filters',
    color: CB_ACCENT,
    icon: '🔍',
    what: 'search_memory() embeds the query string and runs vector search over MemoryBlocks in scope. By default it searches only the current session. Pass session_ids="all" to search across every session for the user. Results are ranked by rel_score (cosine similarity).',
    how: [
      'session.search_memory(query=…) — searches current session only (default)',
      'filters={"session_ids": "all"} — spans all sessions for the user',
      'filters={"relevant_k": 10} — controls number of results returned',
      'filters={"annotations": {"source": "preferences"}} — filter by annotation tags',
      'filters={"start_time": "2025-01-01T00:00:00"} — time-bounded search',
    ],
    limitations: [
      'Search quality depends on the embedding model configured on the server',
      'Cross-session search (session_ids="all") is slower for users with many sessions',
      'rel_score is cosine similarity — not a probability; calibrate thresholds empirically',
    ],
    stack: ['Couchbase vector search', 'FilterOptions', 'MemoryList'],
    questions: [
      { label: 'How do I search only specific sessions, not all of them?', text: null },
      { label: 'How do I filter search results by annotation?', text: null },
      { label: 'How should I inject search results into an LLM prompt?', text: null },
    ],
  },
  'agent-memory-integration': {
    title: 'Integrating the SDK',
    subtitle: 'Install, connect, and wire Agent Memory into a FastAPI agent',
    color: CB_ACCENT,
    icon: '🔧',
    what: 'The SDK is a pure Python HTTP client — install it, point it at a running agentmem server, and call create_user/create_session/add_memory/search_memory. Use AsyncAgentMemoryClient in async FastAPI apps. Handle ConflictError for idempotent user and session creation.',
    how: [
      'pip install -e agentmemory/agentmemory-sdk-main (or from GitHub)',
      'Use AsyncAgentMemoryClient as an async context manager in FastAPI lifespan',
      'Wrap create_user/create_session in try/except ConflictError for idempotent upsert',
      'Call search_memory before LLM generation, inject results into system prompt',
      'Call add_memory after generation to persist the new turn',
    ],
    limitations: [
      'The agentmem server must be running and reachable — the SDK has no offline mode',
      'JWT token required if OIDC is enabled on the server (set AGENTMEMORY_JWT_TOKEN env var)',
      'SDK logging is enabled at INFO by default — call disable_logging() in production',
    ],
    stack: ['AsyncAgentMemoryClient', 'FastAPI', 'agentmem server', 'Couchbase'],
    questions: [
      { label: 'How do I run the agentmem server locally?', text: null },
      { label: 'How do I handle ConflictError when creating users or sessions?', text: null },
